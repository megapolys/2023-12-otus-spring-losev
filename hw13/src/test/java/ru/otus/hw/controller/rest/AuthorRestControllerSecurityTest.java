...
public class AuthorRestControllerSecurityTest extends AbstractControllerTest {
	...
	@Test
	@WithMockUser(roles = {})
	void whenGetAuthorsThenForbidden() throws Exception {
		mockMvc.perform(get("/api/v1/authors"))
	...
	@Test
	@WithMockUser(roles = {})
	void whenGetAuthorThenForbidden() throws Exception {
		mockMvc.perform(get("/api/v1/authors/1"))
			.andExpect(status().isForbidden());
	}
	...
	@Test
	@WithMockUser
	void whenAddAuthorThenForbidden() throws Exception {
		AuthorFormDto author = AuthorFormDtoGenerator.generateNewAuthor();
	...
	@Test
	@WithMockUser
	void whenEditAuthorThenForbidden() throws Exception {
		AuthorFormDto author = AuthorFormDtoGenerator.generate();
	...
	@Test
	@WithMockUser
	void whenDeleteAuthorThenForbidden() throws Exception {
		mockMvc.perform(delete("/api/v1/authors/1")
				.with(csrf()))
			.andExpect(status().isForbidden());
	}
